...
public class Goblin : CombatEntity, IComparable<Goblin>, IEquatable<Goblin>
{
    ...
    public float AttackRange { get; set; } = 1.0f; // Goblins have a 1.0f attack range.
    ...
    /// </summary>
    public Goblin(bool isBossGoblin = false) : this(names[RNG.Next(names.Length)], isBossGoblin)
    { }

    /// <summary>
    /// Creates a new goblin with a pre-seeded random number generator.
    /// </summary>
    public Goblin(string name, bool isBossGoblin = false) : base(name)
    {
        var clans = Enum.GetValues<GoblinClan>();
        Clan = clans[RNG.Next(clans.Length)];
        double elderchance = RNG.NextDouble();
        if (!isBossGoblin && elderchance <= 0.85)           // Most goblins are pretty tame...
        {
        ...
        else if (!isBossGoblin && elderchance > 0.85)            // But sometimes they're stronger!
        {
            SpriteName = "Enemies/Martian";
            AttackPower = RNG.Next(4, 6 + 1);   // Elder goblins have attack-strength 4 at their weakest, up to 6!
            MaxHP = HP = RNG.Next(8, 14 + 1);   // As well as 8-14 health. Stronger than some wizards! :O
        }
        else
        {
            SpriteName = "Enemies/Demon";
            AttackPower = 30;  // Boss goblin has 15 attack power.
            MaxHP = HP = 60;   // Boss goblin has 24 health.
        }
    }
